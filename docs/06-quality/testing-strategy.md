...
| `tests/unit/model-garden.test.ts` | Model identity normalisation (`author/model` and `:variant` to a kebab-case slug); `mergeModelSources` filling nulls without overwriting measured values or popularity; `mergeProviders` preserving a curated group, region and colour over an inferred `other`; the OpenRouter mapper (routed price × 1e6, context window, no invented index, dropping an id with no author) and de-duplication; the Hugging Face mapper and `applyPopularity` enriching matched models, counting unmatched ones and never clearing an existing value |
| `tests/unit/gdelt.test.ts` | `parseGdeltDate` for the GDELT `seendate` format and unusable input; `mapGdeltArticle` canonicalising the URL and dropping a titleless row; the AI-news and isolated world item builders (no fabricated summary and no country guess) |
...
| File | What it covers |
| --- | --- |
| `tests/integration/adapters-contract.test.ts` | Artificial Analysis: `not_configured` without a key and non-retryable; mapping a payload while counting an unusable row as skipped; tolerating an unknown extra field; mapping missing metrics to `null` rather than zero; never guessing a provider group or region; stable persistence ids and a payload hash of at least 16 characters; an envelope without `data` or `models` yielding zero rows and `ok: true`; a transport failure producing a retryable network error; the endpoint is `/data/llms/models`; a response with no pagination block issues exactly one request. OpenRouter: mapping the catalogue and skipping an unusable row; failing loudly on an unexpected envelope. Hugging Face: reading popularity with no key; failing loudly when the payload is not an array. Bluesky: issuing no request with no monitored account; mapping only a monitored author; dropping an unmonitored author. Hacker News: issuing no request with no configured account; mapping points as likes and comments as replies with reposts null; failing loudly on an unexpected envelope. GDELT: mapping articles and skipping a titleless row; treating a response with no articles as a valid empty result. World: `not_configured` without a provider; dropping a non-neutral summary while keeping the attributable item; keeping a neutral summary and flagging a contested story; normalising an unusable date to `null`. Job runner: an unknown job key failing with the known keys listed; every source `deferred` or `disabled` in mock mode with no writes; a disabled source reported as `disabled`; zero writes for every job while Supabase is unconfigured; idempotent outcomes and totals for a repeated run at the same timestamp; and a live-eligible source set per job |
