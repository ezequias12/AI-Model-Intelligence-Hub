    ...
    writeModels(rows) {
      return upsert(
        ...
        rows.map((row) => ({
          ...
          cost_per_task_usd: row.metrics.costPerTaskUsd,
          answer_tokens_per_task: row.metrics.answerTokensPerTask,
          reasoning_tokens_per_task: row.metrics.reasoningTokensPerTask,
